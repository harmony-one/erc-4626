...
import { StakingVaultV1 } from "@contracts/StakingVaultV1.sol";
import "@contracts/Reward.sol";

contract DeployScriptTest is Test {
    Token token;
    StakingVaultV1 vault;
    ...
    function setUp() public {
        ...
        // Deploy StakingVaultV1
        vault = new StakingVaultV1(token, deployer);
    ...
    function testWithdraw() public {
        ...
        uint256 fee = 1 ether;
        uint256 depositMinusFee = 99 ether;
        ...
        assertEq(token.balanceOf(FeeRecepient), fee * 2, 'FeeRecepient balance deposit');
        vm.stopPrank();
        ...
        assertEq(token.balanceOf(FeeRecepient), fee * 3, 'FeeRecepient balance deposit');
        vm.stopPrank();
    ...
    function testWithdrawWithRewards() public {
        ...
        vault.deposit(depositAmount, User1);
        assertEq(token.balanceOf(User1), 0, 'User1 balance after deposit');
        assertEq(vault.balanceOf(User1), 99 ether, 'User1 balance on Vault after deposit');
        ...
        // ???
        assertEq(token.balanceOf(User2), 1088009999999999999997, 'Final User2 balance');
        vm.stopPrank();

        vm.startPrank(User3);   
        vault.redeem(vault.balanceOf(User3), User3, User3);
        // ???
        assertEq(token.balanceOf(User3), 1088009999999999999998, 'Final User3 balance');
        vm.stopPrank();
